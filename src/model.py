...
class Model(object):
	def __init__(self):
		...
		self.tfidf = TfidfVectorizer(max_df=1.0, min_df=1,
                                   stop_words='english', 
                                   lowercase=True)
		pass
...
if __name__ == '__main__':
	...
	X_train, X_test, y_train, y_test = \
	    train_test_split(X, y, train_size=0.75, shuffle=True, stratify=y, random_state=100)
	...
	print('BNB ROC-AUC on Test Set: {}%'.format(round( \
    	roc_auc_score(y_test,model.predict_proba(X_test)[:,1])*100,1)))
